...
class WishlistViewset(viewsets.ModelViewSet):
    ...
    def add(self, request, pk):
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            if not Wishlist.product.through.objects.filter(product_id = product_id):
                wishlist_instance.product.add(product_id)
                message = {"message": "Product successfully added to wishlist."}
    ...
    def remove(self, request, pk):
        ...
        if serializer.is_valid(raise_exception=True):
            product_id = serializer.validated_data['product'][0].id
            if Wishlist.product.through.objects.filter(product_id = product_id):
                wishlist_instance.product.remove(product_id)
                message = {"message": "Product successfully removed from wishlist."}
                return response.Response(message, status=status.HTTP_200_OK)
            else:
